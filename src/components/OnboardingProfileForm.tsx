 ...
 * Shared component for onboarding profile screens (CreatorInfoScreen, TellUsAboutYouScreen).
 * Handles: photo upload, name input, gender selection, date picker (iOS modal + Android native),
 * age validation, progress header, and the Next button.
 */
...
interface OnboardingProfileFormNavigation {
  canGoBack: () => boolean;
  goBack: () => void;
...
type OnboardingProfileFormProps = Readonly<{
  ...
  /** Called with the collected form data when Next is pressed. */
  onNext: (data: {
  ...
  /** Optional sanitizer for the name input text. */
...
const OnboardingProfileForm = ({
  ...
  const handleNameChange = useCallback((text: string) => {
    setName(sanitizeName ? sanitizeName(text) : text);
  }, [sanitizeName]);
  ...
  }, [isFormValid, onNext, profileImage, name, gender, date]);
    ...
    <SafeAreaView style={styles.container}>
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={styles.flex}>
        ...
        <View style={styles.content}>
          ...
          {/* Name Input */}
          <Text style={styles.label}>{nameLabel} <Text style={styles.required}>*</Text></Text>
          <LinearGradient
