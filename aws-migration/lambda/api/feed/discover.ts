  ...
  try {
    ...
    const result = await db.query(
      `SELECT p.id, p.author_id, p.content, p.media_urls, p.media_type, p.tags,
              ...
              pr.id as profile_id, pr.username, CASE WHEN pr.account_type = 'pro_business' AND pr.business_name IS NOT NULL AND pr.business_name != '' THEN pr.business_name ELSE pr.full_name END as full_name, pr.avatar_url, pr.is_verified, pr.account_type,
              ${isLikedExpr},
              ${isSavedExpr}
       FROM posts p
