/**
 * Tests for follow-requests/check-pending Lambda handler
...
import { handler } from '../../follow-requests/check-pending';
...
describe('follow-requests/check-pending handler', () => {
  ...
  describe('happy path', () => {
    it('should return hasPending=false when no pending request exists', async () => {
      const event = makeEvent();
      const result = await handler(event);
    ...
    it('should return hasPending=true when a pending request exists', async () => {
      mockDb.query.mockImplementation((sql: string) => {
        if (typeof sql === 'string' && sql.includes('SELECT id FROM profiles WHERE cognito_sub')) {
          return Promise.resolve({ rows: [{ id: REQUESTER_PROFILE_ID }] });
        }
