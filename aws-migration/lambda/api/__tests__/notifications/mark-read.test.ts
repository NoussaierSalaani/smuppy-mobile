/**
 * Tests for notifications/mark-read Lambda handler
...
import { handler } from '../../notifications/mark-read';
...
describe('notifications/mark-read handler', () => {
  ...
  describe('input validation', () => {
    ...
    it('should return 400 when notification ID is invalid UUID', async () => {
      ...
      const event = makeEvent({ pathParameters: { id: 'not-a-valid-uuid' } });
      const result = await handler(event);
  ...
  describe('happy path', () => {
    ...
    it('should pass notification ID and profile ID to the UPDATE query', async () => {
      mockDb.query.mockResolvedValueOnce({
        rows: [{ id: TEST_NOTIFICATION_ID }],
      });
