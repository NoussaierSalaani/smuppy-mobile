...
const SearchScreen = (): React.JSX.Element => {
  ...
  const handleUserPress = (user: { id: string }): void => {
    navigation.navigate('UserProfile', { userId: user.id });
  };

  const handlePostPress = (_post: ExplorePost): void => {
    // TODO: Navigate to post detail
  };

  const handleRemoveRecentSearch = (_id: string): void => {
    // TODO: Remove from recent searches
  };

  // Split posts into 2 columns for Pinterest effect
  const getColumns = (): { leftColumn: ExplorePost[]; rightColumn: ExplorePost[] } => {
    const leftColumn: ExplorePost[] = [];
    const rightColumn: ExplorePost[] = [];
  ...
  const renderPinterestCard = (item: ExplorePost): React.JSX.Element => (
    <TouchableOpacity
      key={item.id}
      ...
      activeOpacity={0.9}
    >
      <Image source={{ uri: item.image }} style={[styles.pinterestImage, { height: item.height }]} />
      ...
      <View style={styles.cardInfo}>
        ...
        <TouchableOpacity style={styles.cardMeta} onPress={() => handleUserPress(item.user)}>
      ...
      {!isSearchFocused && (
        ...
        >
          <View style={styles.pinterestGrid}>
            <View style={styles.pinterestColumn}>{leftColumn.map(renderPinterestCard)}</View>
            <View style={styles.pinterestColumn}>{rightColumn.map(renderPinterestCard)}</View>
          </View>
        </ScrollView>
      )}
            ...
            <View key={item.id} style={styles.recentItem}>
              ...
              >
                ...
                <View style={styles.recentInfo}>
                  ...
                  {item.subtitle && <Text style={styles.recentNewPosts}>{item.subtitle}</Text>}
                </View>
              </TouchableOpacity>
