/**
 * Tests for posts/likes-batch Lambda handler
...
import { handler } from '../../posts/likes-batch';
...
describe('posts/likes-batch handler', () => {
  ...
  describe('successful batch check', () => {
    it('should return likes map with true for liked posts and false for others', async () => {
      // The query returns which posts are liked
      ...
      expect(body.likes[POST_ID_1]).toBe(true);
      expect(body.likes[POST_ID_2]).toBe(false);
    });

    it('should return all false when no posts are liked', async () => {
      ...
      expect(result.statusCode).toBe(200);
      const body = JSON.parse(result.body);
      expect(body.likes[POST_ID_1]).toBe(false);
      expect(body.likes[POST_ID_2]).toBe(false);
      expect(body.likes[POST_ID_3]).toBe(false);
    });

    it('should query the likes table with correct parameters', async () => {
      mockDb.query.mockResolvedValueOnce({ rows: [] });
      ...
      const queryCall = mockDb.query.mock.calls.find(
        (call: unknown[]) => typeof call[0] === 'string' && (call[0] as string).includes('SELECT post_id FROM likes'),
      );
