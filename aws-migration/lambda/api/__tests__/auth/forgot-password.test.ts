 ...
 * Tests for initiating password reset flow (standalone handler, uses requireRateLimit)
 */
...
jest.mock('@aws-sdk/client-cognito-identity-provider', () => ({
  ...
  ForgotPasswordCommand: jest.fn((params: unknown) => ({ ...params as object, _type: 'ForgotPasswordCommand' })),
...
describe('Forgot Password Handler', () => {
  ...
  beforeAll(async () => {
    ...
    const module = await import('../../auth/forgot-password');
  ...
  describe('Happy Path', () => {
    it('should send reset code and return success', async () => {
      ...
      expect(body.message).toContain('password reset code has been sent');
    });
  ...
  describe('Cognito Error Handling — Anti-Enumeration', () => {
    ...
    it('should return 400 on InvalidParameterException (user not confirmed)', async () => {
      mockCognitoSend.mockResolvedValueOnce({ Users: [{ Username: 'testuser' }] });
      ...
      expect(body.message).toContain('verify your email first');
    });
  });
