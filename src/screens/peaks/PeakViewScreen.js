...
const PeakViewScreen = () => {
  ...
  const currentPeak = peaks[currentIndex] || {};
  ...
  const handlePreviousPeakSameUser = () => {
    if (currentIndex > 0) {
      setCurrentIndex(currentIndex - 1);
    }
  };

  const handleNextPeakSameUser = () => {
    if (currentIndex < peaks.length - 1) {
      setCurrentIndex(currentIndex + 1);
    }
  };

  const handlePeakSelect = (index) => {
    setCurrentIndex(index);
  };

  const handlePeakComplete = () => {
    if (currentIndex < peaks.length - 1) {
      setCurrentIndex(currentIndex + 1);
    }
  };
