...
// Comprehensive expertise categories for Pro Creator accounts - 24 categories total
const ALL_EXPERTISE = [
  ...
  },
  {
    ...
    items: [
      ...
      { name: 'Mental Performance', icon: 'brain-outline', color: '#673AB7' },
...
export default function ExpertiseScreen({ navigation, route }: ExpertiseScreenProps) {
  ...
  const handleNext = useCallback(() => {
    navigate('Guidelines', { ...params, expertise: selected });
  }, [navigate, params, selected]);

  const handleSkip = useCallback(() => {
    navigate('Guidelines', { ...params, expertise: [] });
  }, [navigate, params]);

  const handleExploreMore = useCallback(() => {
    setVisibleCount(prev => Math.min(prev + EXPAND_BY, ALL_EXPERTISE.length));
  }, []);

  const visibleCategories = useMemo(() =>
    ALL_EXPERTISE.slice(0, visibleCount),
    [visibleCount]
  );
