...
// Progress circle (outer green border)
const STROKE_WIDTH = 3;
const CENTER = BUTTON_SIZE / 2;
const RADIUS = (BUTTON_SIZE / 2) - (STROKE_WIDTH / 2); // 48
const CIRCUMFERENCE = 2 * Math.PI * RADIUS;

// S logo size at center (slightly smaller than inner circle)
const S_LOGO_SIZE = 42;

// The "S" path from Smuppy (extracted from SmuppyLogo.tsx)
...
const RecordButton = ({
  ...
  // Animation: 0 = full circle, 1 = empty circle
  ...
  // Animated style - S logo scale (inflate/deflate)
  ...
  const handlePressIn = (): void => {
    ...
    // S logo inflate animation - spring for natural effect
    ...
    }, 50); // 50ms for more precision

    // Circle animation: empties from 0 to 1
    // EXACT duration = maxDuration in milliseconds
    progress.value = 0;
    progress.value = withTiming(1, {
      duration: maxDuration * 1000, // Synchronized EXACTLY with the chosen duration
  ...
  const handlePressOut = (): void => {
    ...
    // S logo deflate animation - spring
    ...
    <View style={styles.wrapper}>
      ...
      >
        <View style={styles.buttonContainer}>
          ...
          {/* Smuppy S logo at center - inflate/deflate */}
