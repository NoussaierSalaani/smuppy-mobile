/**
 * Tests for business/subscription-reactivate Lambda handler
...
import { handler } from '../../business/subscription-reactivate';
...
describe('business/subscription-reactivate handler', () => {
  ...
  it('returns 403 when user does not own the subscription', async () => {
    ...
    mockPool.query.mockResolvedValueOnce({
      rows: [{ id: TEST_SUB_ID, user_id: 'other-user', stripe_subscription_id: null, status: 'active', cancel_at_period_end: true, current_period_end: FUTURE_DATE }],
    });
    const result = await handler(makeEvent());
  ...
  it('returns 400 when subscription is not scheduled for cancellation', async () => {
    ...
    (resolveProfileId as jest.Mock).mockResolvedValue(TEST_PROFILE_ID);
    mockPool.query.mockResolvedValueOnce({
      rows: [{ id: TEST_SUB_ID, user_id: TEST_PROFILE_ID, stripe_subscription_id: null, status: 'active', cancel_at_period_end: false, current_period_end: FUTURE_DATE }],
    });
    const result = await handler(makeEvent());
    expect(result.statusCode).toBe(400);
    expect(JSON.parse(result.body).message).toBe('Subscription is not scheduled for cancellation');
  });

  it('returns 400 when subscription period has ended', async () => {
    const { getUserFromEvent, resolveProfileId } = require('../../utils/auth');
    (getUserFromEvent as jest.Mock).mockReturnValue({ id: TEST_SUB, sub: TEST_SUB });
    (resolveProfileId as jest.Mock).mockResolvedValue(TEST_PROFILE_ID);
    mockPool.query.mockResolvedValueOnce({
      rows: [{ id: TEST_SUB_ID, user_id: TEST_PROFILE_ID, stripe_subscription_id: null, status: 'active', cancel_at_period_end: true, current_period_end: PAST_DATE }],
    });
    const result = await handler(makeEvent());
    expect(result.statusCode).toBe(400);
    expect(JSON.parse(result.body).message).toContain('period has ended');
  });

  it('returns 200 and reactivates subscription successfully', async () => {
    const { getUserFromEvent, resolveProfileId } = require('../../utils/auth');
    (getUserFromEvent as jest.Mock).mockReturnValue({ id: TEST_SUB, sub: TEST_SUB });
    (resolveProfileId as jest.Mock).mockResolvedValue(TEST_PROFILE_ID);
    mockPool.query
      .mockResolvedValueOnce({
        rows: [{ id: TEST_SUB_ID, user_id: TEST_PROFILE_ID, stripe_subscription_id: null, status: 'active', cancel_at_period_end: true, current_period_end: FUTURE_DATE }],
      })
  ...
  it('returns 200 and calls Stripe when stripe_subscription_id exists', async () => {
    ...
    mockPool.query
      .mockResolvedValueOnce({
        rows: [{ id: TEST_SUB_ID, user_id: TEST_PROFILE_ID, stripe_subscription_id: 'sub_stripe_123', status: 'active', cancel_at_period_end: true, current_period_end: FUTURE_DATE }],
      })
      .mockResolvedValueOnce({ rowCount: 1 });
