/**
 * Tests for posts/is-saved Lambda handler
 * Validates auth, UUID validation, profile resolution, saved/unsaved status, and error handling.
 */
...
import { handler } from '../../posts/is-saved';
...
describe('posts/is-saved handler', () => {
  ...
  describe('saved status', () => {
    it('should return saved=true with savedAt when post is saved', async () => {
      const savedAt = '2026-02-16T12:00:00Z';
      mockDb.query.mockResolvedValueOnce({
        rows: [{ id: 'saved-record-id', created_at: savedAt }],
      });
    ...
    it('should return saved=false with savedAt=null when post is not saved', async () => {
    ...
    it('should query saved_posts with correct parameters', async () => {
      mockDb.query.mockResolvedValueOnce({ rows: [] });
      ...
      const savedCall = mockDb.query.mock.calls.find(
        (call: unknown[]) => typeof call[0] === 'string' && (call[0] as string).includes('saved_posts'),
      );
      expect(savedCall).toBeDefined();
      expect(savedCall![1]).toEqual([TEST_PROFILE_ID, TEST_POST_ID]);
    });
  });
