/**
 * Tests for reports/report-peak Lambda handler
 * Uses createReportHandler with validReasons and checkPeakEscalation.
 */
...
describe('reports/report-peak handler', () => {
  ...
  describe('validation', () => {
    it('should return 400 when peakId is missing', async () => {
    ...
    it('should return 400 when reason is invalid (not in VALID_REASONS)', async () => {
      const result = await handler(makeEvent({ body: { peakId: VALID_PEAK_ID, reason: 'invalid_reason' } }));
      expect(result.statusCode).toBe(400);
      expect(JSON.parse(result.body).message).toContain('Invalid report reason');
    });

    it('should accept valid reasons', async () => {
      mockDb.query.mockResolvedValueOnce({ rows: [{ id: VALID_PEAK_ID }] }); // peak exists
      mockClient.query
      ...
      const result = await handler(makeEvent({ body: { peakId: VALID_PEAK_ID, reason: 'harassment' } }));
      expect(result.statusCode).toBe(201);
    });
  });

  describe('peak existence', () => {
  ...
  describe('happy path', () => {
    it('should create report and return 201', async () => {
      mockDb.query.mockResolvedValueOnce({ rows: [{ id: VALID_PEAK_ID }] });
      mockClient.query
        .mockResolvedValueOnce({ rows: [] })
        .mockResolvedValueOnce({ rows: [] })
        .mockResolvedValueOnce({ rows: [{ id: VALID_REPORT_ID }] })
    ...
    it('should call checkPeakEscalation and checkUserEscalation', async () => {
      mockDb.query.mockResolvedValueOnce({ rows: [{ id: VALID_PEAK_ID }] });
      mockClient.query
        .mockResolvedValueOnce({ rows: [] })
        .mockResolvedValueOnce({ rows: [] })
      ...
      expect(checkPeakEscalation).toHaveBeenCalledWith(mockDb, VALID_PEAK_ID);
